...
export function* bleScan(): Generator<*, *, *> {
  yield put(log('BLE scan started...'));
  yield fork(handleScanning, manager);
  yield put(log('BLE scan stopped...'));
}
  ...
  const scanningChannel = yield eventChannel(emit => {
    manager.startDeviceScan(
      ...
      (error, scannedDevice) => {
        ...
        if (scannedDevice != null && scannedDevice.localName != null && scannedDevice.rssi > RSSI_THRESHHOLD && scannedDevice.localName.indexOf("BlueUp") !== -1) {
          emit([error, scannedDevice]);
        }
      },
    );
